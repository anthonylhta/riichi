<script lang="ts">
	...
	// Resolved (non-null) puzzle record, derived from the streamed load promise so
	// we never import the server-only module's types directly.
	type Today = NonNullable<Awaited<PageData['puzzle']>['today']>;

	let { today }: { today: Today } = $props();
	...
	let acceptTiles = $derived(puzzle.ukeireTiles.map((code, id) => ({ code, id, isRed: false })));

	let selected = $state<number | null>(null);
	let revealed = $derived(selected !== null);
	let chosenCode = $derived(selected === null ? null : handTiles[selected].code);
	let isCorrect = $derived(chosenCode !== null && puzzle.bestDiscards.includes(chosenCode));
...
{#if revealed}
	<div class="result" class:ok={isCorrect}>
		...
		<p class="answer">
			Best discard:
			<strong>{puzzle.bestDiscards.map((c) => tileLabel(c)).join(' / ')}</strong>
